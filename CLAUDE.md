...
2. `python3 scripts/bf2stats_to_sql.py data/<folder>` writes `supabase/imports/<folder>/`: `1_players.sql`, `2_new_awards.sql` (only if the dump has award ids missing from the catalog), `3_profiles_N.sql`, `4_rounds.sql` (only if there are new rounds), and `RUN_ORDER.txt`. It reads the live maps, sessions and award catalog through the public REST API (`.env.local` anon key).
